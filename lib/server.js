			  ...
			  console.log('setting zone', m[1], m[2], 'to', message.toString());
				...
				if (m[2] == 'input') {
					...
					var foundInput = inputs.find(function(input) {
					    return input.title === inputTitle;
					});

					if (!foundInput) {
					    console.log("Invalid input title: " + inputTitle);
					    console.log("Available inputs:", inputs.map(i => i.title).join(', '));
					    return;
					}

					zone.setInput(foundInput.title);
...
function onReceive(data) {
	...
	//Zone parameters
		...
		if (allZonesQueried && !unitParametersReceived) {
			...
			unitParametersReceived = true;
			new HomeAssistant(mqtt).publishAutoDiscoveryTopics(UNIT_PARAMETERS, SYSTEM_SETTINGS);
			// Now read preset parameters AFTER HA discovery is set up
			readCurrentPresets();
		}
	}

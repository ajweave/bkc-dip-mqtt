...
function onReceive(data) {
    ...
    console.log('<', data);
	//validate checksum
	// Split on comma respecting quoted strings, then trim trailing
	// semicolon from the final token (also respecting quotes).
	...
	if (lastIdx >= 0) {
		var lastParts = splitRespectingQuotes(parts[lastIdx], ';');
		parts[lastIdx] = lastParts[0];
	}

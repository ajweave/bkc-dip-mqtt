...
* Improve logging
* Make configurable
* Support ethernet and serial.
**/
require('prototypes');
require('./system_settings.js');
require('./unit_parameters.js');
require('./preset_parameters.js');
var Driver = require('./net.js');
//var Driver = require('./serial.js');
...
//Driver.open("/dev/ttyUSB1", discover, onReceive);
Driver.open("172.16.0.110", 23, discover, onReceive);
...
mqtt.on('message', function (topic, message) {
  ...
  if (m != null) {
  	...
  	if (zone) {
		...
		else if (m[2] == 'volume') { //FIXME With net driver, values are not sticking - bouncing back to previous value in UI.

...
function PresetParameters(zoneId) {
	...
	this.setPowerState = function(state) {
		...
		var command = '(' + receiveId + ',S,Z' + this.zoneId + ',24=' + v + ';)';
		console.log('Sending command', command);
 		serialPort.write(command, function(err, bytesWritten) {
			if (err) {
				return console.log('ERROR: ', err.message);
			}
			command = '(' + receiveId + ',G,Z' + zoneId + ',24;)';
			serialPort.write(command, function(err, bytesWritten) {
				if (err) {
					return console.log('ERROR: ', err.message);
				}
...
var serialPort = new SerialPort("/dev/ttyUSB1", {
	  baudrate: 115200,  //changed from 9600 to 115200 in BKC software
	  dataBits: 8,
	  stopBits: 1,
	  xoff: true,
	  parity: 'none', 

...
function onReceive(data) {
    ...
    if (checksumMatch) {
        ...
        // Use the LAST ';' so a ';' inside a quoted value can't shift the parse.
        var semiColon = data.lastIndexOf(';');
        // Serial frames arrive without the '(' wrapper; the unit checksum
        // covers the content only (no leading '(' and no trailing ';').
        var start = data.indexOf('(');
        if (start < 0) start = 0;
        var checksummedStr = data.substring(start, semiColon);
        ...
        if (calculatedChecksum !== receivedChecksum) {
            console.log('Checksum mismatch: calculated ' + calculatedChecksum +
                ' but frame has ' + receivedChecksum +
                ' (checksummed over ' + checksummedStr.length + ' chars)');
            return;
        }
    } else {

...
function onReceive(data) {
    ...
    // validate checksum — message format is (data;CHECKSUM)
    // Extract checksum from end of message (format: data;XXXX or data;XXXX)
    var checksumMatch = data.match(/;([0-9A-Fa-f]{4})\)?$/);
    if (!checksumMatch) {
        console.log("Checksum missing or malformed in message: " + JSON.stringify(data));
        // Intentionally do NOT drop the frame here: some unit modes omit the
        // trailing checksum. Only frames WITH a checksum that FAIL to verify
        // are dropped (see below). Keep this branch non-fatal.
        //return;
    }
    // Validate checksum when present (BKC-DIP frames: (data;CHECKSUM)).
    // If a checksum is present it MUST match; frames without one are logged
    // and still processed (defensive — some modes omit it).
    if (checksumMatch) {
        var openParen = data.indexOf('(');
        var semiColon = data.indexOf(';');
        if (openParen >= 0 && semiColon > openParen) {
            var receivedChecksum = checksumMatch[1].toUpperCase();
            var checksummedStr = data.substring(openParen + 1, semiColon + 1);
            var calculatedChecksum = DIP.calculateChecksum(checksummedStr);
            if (calculatedChecksum !== receivedChecksum) {
                console.log('Checksum mismatch: expected ' + calculatedChecksum + ' but got ' + receivedChecksum + ' for ' + JSON.stringify(checksummedStr));
                return;
            }
        }
    }
